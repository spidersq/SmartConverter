...
public class MethodGenerator extends AbstractGenerator {
    ...
    private List<String> writeMappedListFields(PsiClass psiClass, String fromName, String toName, FieldMappingResult mappingResult) {
        List<String> builder = new ArrayList<>();
        ...
        for (Map.Entry<PsiMethod, PsiMethod> entry : mappingResult.getMappedObjectMap().entrySet()) {
            ...
            String converter = converterName + ".to" + toClass.getName() + "(" + fromName + "." + (fromGetter.getName()) + "())";
    ...
    private List<String> writeMappedObjectFields(PsiClass psiClass, String fromName, String toName, FieldMappingResult mappingResult) {
        List<String> builder = new ArrayList<>();
        ...
        for (Map.Entry<PsiMethod, PsiMethod> entry : mappingResult.getMappedListMap().entrySet()) {
            ...
            String converter = converterName + ".to" + fromClass.getName() + "List(" + fromName + "." + (fromGetter.getName()) + "())";
            builder.add(toName + "." + toSetter.getName() + "(" + converter + ");");
    ...
    private void createFile(PsiClass psiClass, String converterName, PsiClass paramClass, PsiClass returnClass) {
        ...
        if (converterFile == null) {
            ...
            converterFile = PsiFileFactory.getInstance(psiClass.getProject()).createFileFromText(converterName + ".java", JavaLanguage.INSTANCE, converterContent);
    ...
    private PsiMethod findSetter(PsiClass psiClass, String fieldName) {
        PsiMethod[] setters = psiClass.findMethodsByName("set" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1), true);
        if (setters.length == 1) {
            return setters[0];
        }
        return null;
